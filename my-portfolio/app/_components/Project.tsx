...
export const Project = (
  ...
  const alignmentStyles = align === 'left' 
  ? 'flex-row' 
  : 'flex-row-reverse';

return (
    <Section className={`flex ${alignmentStyles} max-md:flex-col items-start gap-4`} {...divProps}>
        <div style={{ position: 'sticky', top: 60, zIndex: 0 }}>
            <Image src={image} alt={name} width={200} height={200} className="w-full h-auto max-w-sm rounded-sm max-md:w-11/12" />
        </div>
